...
if (!existingAdmin) {
  const hashed = bcrypt.hashSync('zyro@2025', 10);
  db.prepare('INSERT INTO admins (username, password) VALUES (?, ?)').run('mugesh', hashed);
  console.log('✅ Default admin created — username: mugesh, password: zyro@2025');
}
...
app.post('/api/orders', async (req, res) => {
  ...
  try {
    db.prepare(`
      INSERT INTO orders (order_id, name, phone, email, product, quantity, description, address, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')
    `).run(orderId, name, phone, email || '', product, quantity || '', description || '', address || '');

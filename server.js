...
app.post('/api/payment/create-order', async (req, res) => {
  ...
  try {
    ...
    if (data.id) {
      res.json({ success: true, razorpay_order_id: data.id, amount: data.amount, currency: data.currency, key_id: keyId });
    } else {
      res.status(500).json({ error: 'Failed to create Razorpay order', details: data });
    }
  } catch(e) {
...
app.post('/api/payment/verify', (req, res) => {
  ...
  if (expectedSig === razorpay_signature) {
    ...
    db.prepare('UPDATE orders SET payment_status = ?, razorpay_id = ?, updated_at = CURRENT_TIMESTAMP WHERE order_id = ?')
      .run('paid', razorpay_payment_id, order_id);
